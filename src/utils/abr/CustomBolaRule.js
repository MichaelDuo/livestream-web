...
import MetricsConstants from 'dashjs/src/streaming/constants/MetricsConstants';
import {HTTPRequest} from 'dashjs/src/streaming/vo/metrics/HTTPRequest';
import Events from 'dashjs/src/core/events/Events';
...
function BolaRule() {
	...
	function setup() {
		...
		eventBus.on(
			...
			onPeriodSwitchStarted,
			instance
		);

		eventBus.on(
			Events.MEDIA_FRAGMENT_LOADED,
			onMediaFragmentLoaded,
			instance
		);
		eventBus.on(
		...
		eventBus.on(
			dashjs.MediaPlayer.events.FRAGMENT_LOADING_ABANDONED,
	...
	function calculateBolaParameters(stableBufferTime, bitrates, utilities) {
		const highestUtilityIndex = utilities.reduce(
			(highestIndex, u, uIndex) =>
				u > utilities[highestIndex] ? uIndex : highestIndex,
			0
		);
		...
		const bufferTime = Math.max(
			stableBufferTime,
			MINIMUM_BUFFER_S +
				MINIMUM_BUFFER_PER_BITRATE_LEVEL_S * bitrates.length
		);
		...
		// giving:
		const gp =
			(utilities[highestUtilityIndex] - 1) /
			(bufferTime / MINIMUM_BUFFER_S - 1);
	...
	function getInitialBolaState(rulesContext) {
		...
		const params = calculateBolaParameters(
			stableBufferTime,
			bitrates,
			utilities
		);
	...
	function checkBolaStateStableBufferTime(bolaState, mediaType) {
		...
		if (bolaState.stableBufferTime !== stableBufferTime) {
			...
			if (params.Vp !== bolaState.Vp || params.gp !== bolaState.gp) {
				...
				let effectiveBufferLevel =
					bufferLevel + bolaState.placeholderBuffer;
				...
				bolaState.placeholderBuffer = Math.max(
					0,
					effectiveBufferLevel - bufferLevel
				);
			}
		}
	}
	...
	function getQualityFromBufferLevel(bolaState, bufferLevel) {
		...
		for (let i = 0; i < bitrateCount; ++i) {
			let s =
				(bolaState.Vp * (bolaState.utilities[i] + bolaState.gp) -
					bufferLevel) /
				bolaState.bitrates[i];
		...
		for (let i = quality - 1; i >= 0; --i) {
			...
			if (bolaState.utilities[i] < bolaState.utilities[quality]) {
				...
				const level =
					bolaState.Vp *
					(bolaState.gp +
						(qBitrate * iUtility - iBitrate * qUtility) /
							(qBitrate - iBitrate));
	...
	function onBufferEmpty() {
		...
		for (const mediaType in bolaStateDict) {
			if (
				...
				bolaStateDict[mediaType].state === BOLA_STATE_STEADY
			) {
	...
	function onMediaFragmentLoaded(e) {
		if (e && e.chunk && e.chunk.mediaInfo) {
			...
			if (bolaState && bolaState.state !== BOLA_STATE_ONE_BITRATE) {
				...
				if (
					isNaN(bolaState.mostAdvancedSegmentStart) ||
					start > bolaState.mostAdvancedSegmentStart
				) {
	...
	function onMetricAdded(e) {
		if (
			e &&
			e.metric === MetricsConstants.HTTP_REQUEST &&
			e.value &&
			e.value.type === HTTPRequest.MEDIA_SEGMENT_TYPE &&
			e.value.trace &&
	...
	function checkNewSegment(bolaState, mediaType) {
		if (
			!isNaN(bolaState.lastSegmentStart) &&
			!isNaN(bolaState.lastSegmentRequestTimeMs) &&
			!isNaN(bolaState.placeholderBuffer)
		) {
			...
			if (!isNaN(bolaState.lastSegmentFinishTimeMs)) {
				...
				const bufferAtLastSegmentRequest =
					bufferLevel +
					0.001 *
						(bolaState.lastSegmentFinishTimeMs -
							bolaState.lastSegmentRequestTimeMs); // estimate
				const maxEffectiveBufferForLastSegment = maxBufferLevelForQuality(
					bolaState,
					bolaState.lastQuality
				);
				const maxPlaceholderBuffer = Math.max(
					0,
					maxEffectiveBufferForLastSegment -
						bufferAtLastSegmentRequest
				);
				bolaState.placeholderBuffer = Math.min(
					maxPlaceholderBuffer,
					bolaState.placeholderBuffer
				);
			}
			...
			if (
				bolaState.lastSegmentWasReplacement &&
		...
		if (e) {
			...
			if (bolaState && bolaState.state !== BOLA_STATE_ONE_BITRATE) {
				...
				if (bolaState.abrQuality > 0) {
					...
					wantEffectiveBufferLevel = minBufferLevelForQuality(
						bolaState,
						bolaState.abrQuality
					);
				} else {
				...
				const maxPlaceholderBuffer = Math.max(
					0,
					wantEffectiveBufferLevel - bufferLevel
				);
				bolaState.placeholderBuffer = Math.min(
					bolaState.placeholderBuffer,
					maxPlaceholderBuffer
	...
	function getMaxIndex(rulesContext) {
		...
		const throughput = throughputHistory.getAverageThroughput(
			mediaType,
			isDynamic
		);
		const safeThroughput = throughputHistory.getSafeAverageThroughput(
			mediaType,
			isDynamic
		);
		...
		switch (bolaState.state) {
			case BOLA_STATE_STARTUP:
				quality = abrController.getQualityForBitrate(
					mediaInfo,
					safeThroughput,
					latency
				);
				...
				bolaState.placeholderBuffer = Math.max(
					0,
					minBufferLevelForQuality(bolaState, quality) - bufferLevel
				);
			...
			case BOLA_STATE_STEADY:
				...
				const qualityForThroughput = abrController.getQualityForBitrate(
					mediaInfo,
					safeThroughput,
					latency
				);
				if (
					quality > bolaState.lastQuality &&
					quality > qualityForThroughput
				) {
				...
				let delayS = Math.max(
					...
					bufferLevel +
						bolaState.placeholderBuffer -
						maxBufferLevelForQuality(bolaState, quality)
				);
				...
				switchRequest.reason.placeholderBuffer =
					bolaState.placeholderBuffer;
...
BolaRule.__dashjs_factory_name = 'CustomBolaRule';
export default dashjs.FactoryMaker.getClassFactory(BolaRule);
